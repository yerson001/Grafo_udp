...
void read(string& structure){
    ...
    for(int i=0;i<number_attributes;i++){
        int size_atr= string_int(structure.substr(0,3));structure=structure.substr(3,structure.size());
        int size_valor=string_int(structure.substr(0,3));structure=structure.substr(3,structure.size());
        string name_valor=structure.substr(0,size_atr);structure=structure.substr(size_atr,structure.size());
        string valor=structure.substr(0,size_valor);structure=structure.substr(size_valor,structure.size());
        campos+="'"+valor+"',";
    ...
    for(int i=0;i<number_relations;i++){
        int size_rel= string_int(structure.substr(0,3));structure=structure.substr(3,structure.size());
        string n_node=structure.substr(0,size_rel);structure=structure.substr(size_rel,structure.size());
        campos+="'"+n_node+"',";
    }
    relaciones = campos;
    ...
    for(int i=0; i<nombre_atributos.size(); i++){cout<<nombre_atributos[i]<<endl;}
    for(int i=0; i<valor_atributos.size(); i++){cout<<valor_atributos[i]<<endl;}
    for(int i=0; i<valor_relaciones.size(); i++){cout<<valor_relaciones[i]<<endl;}
